...
async function getUser() {
  getRepos();
  //Query.value busca o valor inserido no input(pesquisa)
  const URL = `https://api.github.com/users/${QUERY.value}`;
  const DATA = await fetch(URL);
  const USER_DATA = await DATA.json(); // trazer em formato de Json

  // verificar se o usuário existe
  if (USER_DATA.message === "Not Found") {
    alert("User not Found");
    return; // caso não exista  o processo é morto
  }
...
async function getRepos() {
  ...
  // Vincular a constante REPOS que faz a busca ao array de repositórios 
  USER.repos = REPOS;

}

function getAllData() {
  //função para retornar o objeto em html
  const USER_INFO = document.getElementById("user-info");
  USER_INFO.getAttribute("style", "display: block");
  ...
  USER_INFO.innerHTML = `
    ...
    ${USER.repos.map((repo) => `<div class="repo"> 
      ...
      <p class="repo-languages">${repo.languages == null ? (repo.languages = "No languages") : repo.languages} </p>
